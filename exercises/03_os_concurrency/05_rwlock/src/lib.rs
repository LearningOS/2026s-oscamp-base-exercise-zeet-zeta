...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if x & WRITER_HOLDING != 0 || x & WRITER_WAITING != 0 {
                core::hint::spin_loop();
                continue;
            };
            if x & READER_MASK == READER_MASK {
                core::hint::spin_loop();
                continue;
            }
            match self
                .state
                .compare_exchange(x, x + 1, Ordering::Acquire, Ordering::Relaxed)
            {
...
impl<T> Drop for RwLockReadGuard<'_, T> {
    fn drop(&mut self) {
        // 需要使用 release
        // 可能读取操作还没有完成，如果下一个线程是写者，可能在读取操作真正完成之前就开始修改内存
