...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        let stack_top_aligned = (stack_top - 16) & !15;
        let mut context = TaskContext::default();
        context.ra = thread_wrapper as *const () as u64;
        context.sp = stack_top_aligned as u64;
    ...
    fn schedule_next(&mut self) {
        ...
        if let Some(new_index) = next_index {
            if self.threads[old_index].state == ThreadState::Running {
                self.threads[old_index].state = ThreadState::Ready;
            }
            self.threads[new_index].state = ThreadState::Running;
            self.current = new_index;

            unsafe {
                CURRENT_THREAD_ENTRY = self.threads[new_index].entry.take();
            }

            let old_ctx = self.threads[old_index].ctx.as_mut_ptr();
            let new_ctx = self.threads[new_index].ctx.as_ptr();
            unsafe {
