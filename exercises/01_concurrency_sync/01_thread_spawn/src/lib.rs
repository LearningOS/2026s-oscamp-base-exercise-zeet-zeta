...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let handle = std::thread::spawn(|| numbers.into_iter().map(|x| x * 2).collect());
    handle.join().unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let handle1 = std::thread::spawn(|| a.into_iter().sum());
    let handle2 = std::thread::spawn(|| b.into_iter().sum());
