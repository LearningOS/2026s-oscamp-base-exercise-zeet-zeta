...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            if pte & PTE_V == 0 {
                ...
                node.entries[vpn] = (new_ppn << 10) | PTE_V;
                current_ppn = new_ppn;
            } else {
                current_ppn = (pte >> 10) & ((1 << 44) - 1);
            }
        }
        let vpn0 = Sv39PageTable::extract_vpn(va, 0);
        let leaf_node = self.nodes.get_mut(&current_ppn).unwrap();
        leaf_node.entries[vpn0] = (pa >> 12 << 10) | flags | PTE_V;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            let vpn = Sv39PageTable::extract_vpn(va, level);
            let node;
            match self.nodes.get(&current_ppn) {
                Some(n) => node = n,
                None => {
                    panic!("err {current_ppn} {level}\n");
                }
            }
            let pte = node.entries[vpn];
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                let pa = (ppn << 12) | (va & (1 << 12 + level * 9) - 1);
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte2 = self.nodes.get(&current_ppn).unwrap().entries[vpn2];
        ...
        if (pte2 & PTE_V) == 0 {
            ...
            current_ppn = (pte2 >> PPN_SHIFT) & ((1 << 44) - 1);
        }

        let vpn1 = Self::extract_vpn(va, 1);
        let node = self.nodes.get_mut(&current_ppn).unwrap();
        node.entries[vpn1] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    }
}
