...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        let entry = self
            .entries
            .iter()
            .find(|x| x.valid && x.vpn == vpn && x.asid == asid);
    ...
    pub fn flush_all(&mut self) {
        ...
        self.entries.iter_mut().for_each(|x| x.valid = false);
    }
    ...
    pub fn valid_count(&self) -> usize {
        ...
        self.entries.iter().filter(|x| x.valid == true).count()
    }
}
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        self.tlb.lookup(vpn, self.current_asid).or_else(|| {
            self.page_table
                .iter()
                .find(|(asid, pm)| *asid == self.current_asid && pm.vpn == vpn)
                .map(|(_, pm)| {
